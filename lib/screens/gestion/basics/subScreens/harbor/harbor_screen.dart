...
class _HarborScreen extends State<HarborScreen> {
  ...
  Future<void> fetchHarbor({bool reset = false}) async {
    ...
    try {
      final result = await _harborServices.findAll(page: currentPage);
      ...
      setState(() {
        ...
        _filteredHarbor = List.from(_allHarbor);
  ...
  void searchHarbor(String query) {
    setState(() {
      _filteredHarbor =
          _allHarbor.where((port) {
            final searchPackage = port.name!.toLowerCase().contains(
              query.toLowerCase(),
            );
